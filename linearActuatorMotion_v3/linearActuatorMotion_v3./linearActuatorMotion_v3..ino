...
const int MotorEnabler_1 = 4; // Connected to H-bridge pin 1,2EN
const int MotorGnd_2 = 5;  //Connected to H-bridge pin 4A
const int MotorPower_2 = 6; //Connected to H-bridge pin 3A
const int MotorEnabler_2 = 7; //Connected to H-bridge pin 3,4EN
...
void setup() {
  ...
  digitalWrite(MotorEnabler_1, HIGH);
  digitalWrite(MotorEnabler_2, HIGH);
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (positionInput == 'C'){ //If command input for GET CURRENT POSITION
      getCurrentPosition();
    }    
